...
const handleSignUp = async (e) => {
    ...
    try {
        ...
        const { data, error } = await sb.auth.signUp({ // Corregido: sb.auth
            email,
            password,
        });
        ...
        if (role === 'seller' && logoFile) {
            ...
            const { error: uploadError } = await sb.storage // Corregido: sb.storage
                .from('logos') 
                .upload(fileName, logoFile);

            if (uploadError) throw uploadError;
            
            const { data: urlData } = sb.storage // Corregido: sb.storage
                .from('logos')
        ...
        const { error: profileError } = await sb // Corregido: sb
            .from('profiles')
            .insert([
                { 
                    id: user.id, 
...
const handleLogin = async (e) => {
    ...
    try {
        const { data, error } = await sb.auth.signInWithPassword({ // Corregido: sb.auth
            email,
            password,
        });
        ...
        const { data: profile, error: profileError } = await sb // Corregido: sb
            .from('profiles')
            .select('role')
            .eq('id', data.user.id)
            .single();
...
const handleRecovery = async (e) => {
    ...
    try {
        const { error } = await sb.auth.resetPasswordForEmail(email, { // Corregido: sb.auth
            ...
            redirectTo: window.location.origin + '/new_password.html', 
        });
...
const checkSession = async () => {
    const { data: { session } } = await sb.auth.getSession(); // Corregido: sb.auth
    
    if (session) {
        // Si hay una sesión activa, verificamos el rol y redirigimos
        const { data: profile, error } = await sb // Corregido: sb
            .from('profiles')
            .select('role')
